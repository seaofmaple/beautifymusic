<template>
  <div id="p-list">
    ...
    <div class="list">
      <div class="list-item" v-for="(item, index) in songs" :key="index" @dblclick="play(item.id)">
        <i class="el-icon-caret-right" v-show="currMusic == item.id"></i>
...
export default {
  ...
  computed: {
    total() {
      return this.$store.getters.showPlayList.length
    },
    currMusic() {
  ...
  methods: {
    clearAll() {
      ...
      this.$store.dispatch("saveNowDuration", 0);
      this.$emit('clearAll')
    },
    play(id) {
